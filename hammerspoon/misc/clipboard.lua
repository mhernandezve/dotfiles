--[[
   From https://github.com/miguelso/.hammerspoon/blob/master/tools/clipboard.lua
...
local clipboard_history = settings.get("so.miguel.hs.jumpcut") or {} --If no history is saved on the system, create an empty history
...
function clearAll()
   ...
   clipboard_history = {}
   settings.set("so.miguel.hs.jumpcut",clipboard_history)
...
function pasteboardToClipboard(item)
   ...
   settings.set("so.miguel.hs.jumpcut",clipboard_history) -- updates the saved history
...
hs.hotkey.bind(Hyper, "v", function() jumpcut:popupMenu(hs.mouse.getAbsolutePosition()) end)
